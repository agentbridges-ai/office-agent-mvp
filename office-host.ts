...
let destroyed = false;
let suppressedNativeSaveCount = 0;
...
async function postNativeSave(file: File): Promise<void> {
  if (!parentOrigin || !sessionId) return;
  const buffer = await file.arrayBuffer();
  window.parent.postMessage(
    {
      source: HOST_SAVE_SOURCE,
      sessionId,
      fileName: file.name,
      mimeType: file.type,
      buffer,
...
async function handleInit(message: Extract<OfficeHostParentMessage, { type: 'INIT' }>): Promise<void> {
  ...
  try {
    ...
    const runtimeOptions: RuntimeOptions = {
      ...
      onSave: (file) => {
        if (suppressedNativeSaveCount > 0) {
          suppressedNativeSaveCount -= 1;
          return;
        }
        void postNativeSave(file).catch((error) => {
          postError('save', error);
        });
      },
      onError: (error) => {
...
async function handleSave(message: Extract<OfficeHostParentMessage, { type: 'SAVE' }>): Promise<void> {
  let suppressingNativeSave = false;
  try {
    if (!editor) {
      throw new Error('Editor is not open');
    }
    suppressedNativeSaveCount += 1;
    suppressingNativeSave = true;
    const file = await editor.save(message.targetExt);
    suppressingNativeSave = false;
    const buffer = await file.arrayBuffer();
    postPortMessage(
      {
        protocol: OFFICE_HOST_PROTOCOL,
    ...
    if (suppressingNativeSave && suppressedNativeSaveCount > 0) {
      suppressedNativeSaveCount -= 1;
    }
