(function () {
  ...
  function discoverSystemPlugins(editor) {
    ...
    if (!window.Asc || typeof window.Asc.eak !== 'function') {
      throw new Error('PPT system plugin loader is unavailable.');
    }
    window.Asc.eak(editor);
    systemPluginsDiscovered = true;
  }

  function startSystemPlugins() {
    if (!window.mg || typeof window.mg.z9i !== 'function') {
      throw new Error('PPT system plugin starter is unavailable.');
    }
    if (!window.mg.KEb || !window.mg.KEb.length) return false;
    window.mg.z9i();
    return true;
  }
  ...
  function dispatch(action, payload) {
    ...
    if (!editor) return fail('ONLYOFFICE presentation editor is not ready yet.', 'partial');
    if (action === 'pptGetContext') return ok({ documentType: 'presentation', canSave: hasMethod(editor, ['asc_Save']) });
  ...
  function announceReady() {
    ...
    post({ type: 'ready', result: { documentType: 'presentation', canSave: hasMethod(editor, ['asc_Save']) } });
  }
