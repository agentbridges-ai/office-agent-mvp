...
| x2t build pipeline (patch series repo-owned) | `tools/x2t-wasm/` — 32 patches + scripts; default source is CryptPad modified core (vanilla+patches path documented, not default); WASM bit-identical `e166c252...` |
...
| **vanilla core 默认独立构建** | 默认构建源为 CryptPad modified core。32 patches repo-owned，vanilla+patches 路径已验证但非默认 | 后续工程 |
...
>
> 3. **x2t 构建**: 32 patches + 6 stubs 已 repo-owned。默认构建源为 CryptPad modified core（可复现 bit-identical artifact）。vanilla core + apply-all.sh 路径已研究验证但非默认。
...
- [x] x2t WASM 9.3.0.140 (self-built, bit-identical verified; 32 patches repo-owned; default source: CryptPad modified core)
