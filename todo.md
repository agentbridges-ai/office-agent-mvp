...
| 字体管线 (26 fonts: Latin+CJK+Emoji+Arabic) | `fonts/manifest.json` + `fonts/hash-lock.json` + `bin/verify-font-pack.mjs`, 26 match 0 mismatch (font pack integrity; visual fidelity not claimed) |
| x2t build pipeline (vanilla core default) | `tools/x2t-wasm/` — 32 patches repo-owned; default: vanilla ONLYOFFICE/core + apply-all.sh → bit-identical `e166c252...`; CryptPad fallback retained |
...
| 项 | 原因 | 分类 |
|----|------|------|
| **all ONLYOFFICE 9.3 formats supported** | FB2/OFD 已知未链接; HWP/VSDX/iWork 等未验证 | 扩展工程 |
| **PDF export** | 架构限制 — 需要 server-side conversion。浏览器端被明确阻断 (lib/onlyoffice-compat/pdf.ts)。E2E 验证为负向能力 | 独立项目 |
| **XLS/PPT binary 输出** | x2t WASM read-only — DocFormatLib/XlsFormatLib 链接通过但写入路径挂死 | 不可完成 |
| ~~vanilla core 默认独立构建~~ | ~~was: CryptPad default~~ | ✅ 已升级: clone-core.sh 默认 vanilla + apply-all.sh; CryptPad fallback 保留 |
...
>
> 3. **x2t 构建**: 默认从 vanilla ONLYOFFICE/core v9.3.0.140 + 32 patches 独立构建，产出 bit-identical x2t.wasm。CryptPad fork 保留为 fallback，不再是默认源。
...
- [x] **R3-远期**: Patch series 已 repo-owned ✅
...
- [x] x2t WASM 9.3.0.140 (self-built, bit-identical verified; 32 patches repo-owned; default: vanilla core + apply-all.sh)
...
| 6 | font verify | `node bin/verify-font-pack.mjs` | ✅ 26 match, 0 mismatch |
