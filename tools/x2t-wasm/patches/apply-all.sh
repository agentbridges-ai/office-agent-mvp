...
if [ "$FAILED" -gt 0 ] || [ "$SKIPPED" -gt 0 ]; then
  ...
  if [ "$SKIPPED" -gt 0 ]; then
    echo "ERROR: $SKIPPED patches were skipped (target files not in core)."
  ...
  echo "Core is NOT ready for build."
  echo "Try: X2T_CORE_MODE=cryptpad ./scripts/clone-core.sh for CryptPad fallback."
  exit 1
fi

echo "=== All patches applied ($APPLIED OK, 0 skipped, 0 failed) ==="
